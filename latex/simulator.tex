...
\smallskip
The code is divided into \num{4} main parts: topology, core, utilities, and highly optimized data structures.
Both the core and the topology parts define their own events, messages, initializers, observers and protocols.
...
\begin{itemize}
	...
	\item when a connection is established, nodes exchange \texttt{Addr} messages; at the same time, nodes start to exchange \texttt{Ping} and \texttt{Pong} messages to maintain the newly build topology, as described in \cref{sub:peer-cleanup};
...
Each node in the simulator records a number of metrics, which are periodically connected and aggregated by some observers:
\begin{itemize}
	\item number of messages sent to other peers, grouped by type;
	\item number of nodes removed because of timeouts;
	\item overall distribution of the number of connections established by each node (both incoming and outgoing).
\end{itemize}
The metrics are mainly to check if the implementation of the topology layer is correct, or, at least, its behavior is reasonable.
